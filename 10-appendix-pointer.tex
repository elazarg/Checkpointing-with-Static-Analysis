...
\inferrule*[right=Var]
  { o = P[\mathtt{LOCALS}][v] }
  { \mathsf{Expr}^\#(\Sigma, v, \ell) = (\{o\}, T[o], \bot_\mathcal{D}) }
...
\inferrule*[right=Assign-Var]
  ...
  { \mathsf{Trans}^\#(\Sigma,\mathsf{Assign}(x,e,\mathsf{false}),\ell) = (P[\mathtt{LOCALS}][x \mapsto O],T[O \mapsto \tau],D \sqcup \Delta) } \\
\inferrule*[right=Assign-Subscr]
  ...
  { \mathsf{Trans}^\#(\Sigma, \mathsf{Assign}(\mathsf{Subscript}(v_1, v_2), v, \mathsf{false}), \ell)
    = (P', T, D') } \\
\inferrule*[right=Assign-Attr]
  ...
  { \mathsf{Trans}^\#(\Sigma,\mathsf{Assign}(\mathsf{Attribute}(v_1,f),v,\mathsf{false}),\ell)
    = (P',T,D') }
\end{mathpar}
