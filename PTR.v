...
Module PointerAnalysis.
  ...
  Module AbstractObject.
    ...
    Proof.
      ...
      - destruct c, c0; try discriminate; try (left; reflexivity);
  ...
  Module Analysis (TS : TypeSystemSig).
    ...
    Definition points_to_lookup (P : PointsTo) (o : AbstractObject.t) (f : FieldKey) : ObjectSet.t :=
      ...
      | Some fm =>
          FieldMap.fold (fun k objs acc =>
            if field_matches k f || field_matches f k then 
    ...
    Definition points_to_update (P : PointsTo) (o : AbstractObject.t) (f : FieldKey)
      ...
      let fm := match AOMap.find o P with
               | Some fm => fm
               | None => FieldMap.empty _
               end in
      AOMap.add o (FieldMap.add f objs fm) P.
    ...
    Definition empty_abstract : AbstractState := {|
      as_points_to := empty_points_to;
      as_types := empty_type_map;
      as_dirty := empty_dirty_map
    |}.
    
    Definition empty_state : State := {|
      st_abstract := empty_abstract;
    ...
    Definition eval_const (c : ConstV) : (ObjectSet.t * TypeExpr) :=
      let ty := literal_type c in
      (immutable_const c, ty).
    
    (* Attribute lookup with bound method creation *)
    Definition eval_attribute (st : AbstractState) (var_objs : ObjectSet.t) 
                             (attr : string) (pp : ProgramPoint) 
                             : (AbstractState * ObjectSet.t * TypeExpr) :=
      let var_type := get_types (as_types st) var_objs in
      let attr_type := subscr var_type attr in
      
      let result_type := 
        if is_property attr_type then
          get_return attr_type
        else
          attr_type in
      ...
      let (st', result_objs) :=
        if type_is_immutable result_type then
          ...
          (* functions behind this attribute on each receiver *)
          let func_objs :=
            ObjectSet.fold (fun o acc =>
              ObjectSet.union acc (points_to_lookup (as_points_to st) o (FKName attr))
            ) var_objs ObjectSet.empty in
          let P0 := upd_points_to (as_points_to st) (ObjectSet.singleton bm) self_field var_objs in
          let P1 := upd_points_to P0 (ObjectSet.singleton bm) func_field func_objs in
          let T' := type_update (as_types st) bm attr_type in
          ({| as_points_to := P1; as_types := T'; as_dirty := as_dirty st |},
           ObjectSet.singleton bm)
        else if any_new attr_type && all_new attr_type then
          (* Property that always allocates *)
          let new_obj := alloc_site pp in
          let T' := type_update (as_types st) new_obj result_type in
          ({| as_points_to := as_points_to st; as_types := T'; as_dirty := as_dirty st |},
           ObjectSet.singleton new_obj)
        else
          (* Read from field *)
          let field_objs := ObjectSet.fold (fun o acc =>
            ObjectSet.union acc (points_to_lookup (as_points_to st) o (FKName attr))
          ) var_objs ObjectSet.empty in
          if any_new attr_type then
            ...
            let T' := type_update (as_types st) new_obj result_type in
            ({| as_points_to := as_points_to st; as_types := T'; as_dirty := as_dirty st |},
             ObjectSet.union field_objs (ObjectSet.singleton new_obj))
          else
            (st, field_objs) in
      
      (st', result_objs, result_type).
      
    (* Collect values behind wildcard of a container set (tuples/dicts). *)
    ...
    (* Read bound-call state off a callee object *)
    ...
    Definition join_fieldmaps (fm1 fm2 : FieldMap.t ObjectSet.t) : FieldMap.t ObjectSet.t :=
      FieldMap.fold (fun k objs acc =>
        ...
        FieldMap.add k (ObjectSet.union old objs) acc
      ) fm2 fm1.
    ...
    Definition join_dirty_field (b1 b2 : bool) := orb b1 b2.
    ...
    Definition join_abs (s1 s2 : AbstractState) : AbstractState :=
      {| as_points_to := join_points_to (as_points_to s1) (as_points_to s2);
         as_types     := join_type_maps (as_types s1)     (as_types s2);
         as_dirty     := join_dirty_maps (as_dirty s1)    (as_dirty s2) |}.
    
    (* ---------- Per-callee evaluator ---------- *)
    
    (* One callee object -> apply bound-call semantics and side-effect summary. *)
    Definition eval_call_single (st : AbstractState) (callee : AbstractObject.t) (pp : ProgramPoint)
      : (AbstractState * ObjectSet.t * TypeExpr) :=
      let P := as_points_to st in
      let T := as_types st in
      let callee_set := ObjectSet.singleton callee in
    
      (* Read bound state from the callee object *)
      ...
      (* Argument typing for TS.partial (coarse but sound) *)
      ...
      let st' :=
        ...
        | Some up_ty =>
            ...
            {| as_points_to := P'; as_types := T'; as_dirty := D' |}
        | None => st
        end in
    
      (* Type fresh allocs and wire aliasing into them when requested *)
      let st'' :=
        if se_new effect then
          let T'' := type_update (as_types st') (alloc_site pp) ret_ty in
          {| as_points_to := as_points_to st'; as_types := T''; as_dirty := as_dirty st' |}
        else st' in
      let st''' :=
        if se_new effect && se_points_to_args effect then
          let ali := ObjectSet.union pos_vals kw_vals in
          let P'' := upd_points_to (as_points_to st'') (ObjectSet.singleton (alloc_site pp)) wildcard ali in
          {| as_points_to := P''; as_types := as_types st''; as_dirty := as_dirty st'' |}
        else st'' in
    
      (st''', res_objs, ret_ty).
    
    (* ---------- Multi-callee aggregator ---------- *)
    
    (* Possibly-many callees -> fold eval_call_single and join states/types/objects/types. *)
    Definition eval_call (st : AbstractState) (func_objs : ObjectSet.t) (pp : ProgramPoint)
      : (AbstractState * ObjectSet.t * TypeExpr) :=
      ObjectSet.fold
        (fun f acc =>
    ...
    Definition transfer_load_const (st : State) (dst : StackVar) (c : ConstV) : State :=
      ...
      let T' := type_update_set (as_types (st_abstract st)) objs ty in
      {| st_abstract := {| as_points_to := as_points_to (st_abstract st);
                          as_types := T';
                          as_dirty := as_dirty (st_abstract st) |};
         st_stack := stack_update (st_stack st) dst objs |}.
    
    Definition transfer_load_local (st : State) (dst : StackVar) (name : string) : State :=
      let objs := get_local (as_points_to (st_abstract st)) name in
      {| st_abstract := st_abstract st;
         st_stack := stack_update (st_stack st) dst objs |}.
    
    Definition transfer_load_global (st : State) (dst : StackVar) (name : string) : State :=
      let objs := get_global (as_points_to (st_abstract st)) name in
      {| st_abstract := st_abstract st;
         st_stack := stack_update (st_stack st) dst objs |}.
    ...
    Definition transfer_set_local (st : State) (name : string) (src : StackVar) : State :=
      ...
      {| st_abstract := {| as_points_to := P';
                          as_types := as_types (st_abstract st);
                          as_dirty := as_dirty (st_abstract st) |};
         st_stack := st_stack st |}.
    ...
    Definition transfer_get_attr (st : State) (dst : StackVar) (obj : StackVar) 
      ...
      let (st', result_objs, result_type) := eval_attribute (st_abstract st) obj_set attr pp in
      let T' := type_update_set (as_types st') result_objs result_type in
      {| st_abstract := {| as_points_to := as_points_to st';
                          as_types := T';
                          as_dirty := as_dirty st' |};
         st_stack := stack_update (st_stack st) dst result_objs |}.
    ...
    Definition transfer_set_attr (st : State) (obj : StackVar) (attr : string) 
      ...
      let D' := mark_dirty_set (as_dirty (st_abstract st)) targets (FKName attr) in
      {| st_abstract := {| as_points_to := P';
                          as_types := as_types (st_abstract st);
                          as_dirty := D' |};
         st_stack := st_stack st |}.
    ...
    Definition transfer_construct_tuple (st : State) (dst : StackVar) (elems : list StackVar)
      ...
      ) indexed_elems (as_points_to (st_abstract st)) in
      let T' := type_update (as_types (st_abstract st)) tup 
                           (get_return make_tuple_constructor) in
      {| st_abstract := {| as_points_to := P';
                          as_types := T';
                          as_dirty := as_dirty (st_abstract st) |};
         st_stack := stack_update (st_stack st) dst (ObjectSet.singleton tup) |}.
    ...
    Definition transfer_construct_dict (st : State) (dst : StackVar) 
      ...
      ) pairs (as_points_to (st_abstract st)) in
      let T' := type_update (as_types (st_abstract st)) dict 
                           (get_return make_dict_constructor) in
      {| st_abstract := {| as_points_to := P';
                          as_types := T';
                          as_dirty := as_dirty (st_abstract st) |};
         st_stack := stack_update (st_stack st) dst (ObjectSet.singleton dict) |}.
    ...
    Definition transfer_bind (st : State) (dst func_var args_var kwargs_var : StackVar)
      ...
      (* Wire bound callable structure *)
      let P1 := upd_points_to (as_points_to (st_abstract st)) 
                              (ObjectSet.singleton bound) func_field func_objs in
      ...
      (* Type the bound object *)
      let T' := type_update (as_types (st_abstract st)) bound 
                           (get_types (as_types (st_abstract st)) func_objs) in
      
      {| st_abstract := {| as_points_to := P4;
                          as_types := T';
                          as_dirty := as_dirty (st_abstract st) |};
         st_stack := stack_update (st_stack st) dst (ObjectSet.singleton bound) |}.
    ...
    Definition transfer_unpack (st : State) (dsts : list StackVar) (src : StackVar) : State :=
      ...
      let S' := List.fold_left (fun acc pair =>
        let (idx, dst) := pair in
        let objs := index_lookup (as_points_to (st_abstract st)) src_objs idx in
    ...
    Definition transfer_call (st : State) (dst : StackVar) (func : StackVar) (pp : ProgramPoint) : State :=
      ...
      let (st', result_objs, result_type) := eval_call (st_abstract st) func_objs pp in
      let T' := type_update_set (as_types st') result_objs result_type in
      {| st_abstract := {| as_points_to := as_points_to st';
                          as_types := T';
                          as_dirty := as_dirty st' |};
    ...
    Definition transfer (st : State) (instr : Instruction) (pp : ProgramPoint) : State :=
      ...
      | IMov d s => 
          {| st_abstract := st_abstract st;
             st_stack := stack_update (st_stack st) d (stack_lookup (st_stack st) s) |}
      ...
      (* Pure lookups - mark as immutable *)
      | ILookupDunderUnary d _ _ | ILookupDunderBinary d _ _ _ | ILookupOverload d _ _ _ =>
          let imm := immutable_type type_bot in
          {| st_abstract := st_abstract st;
             st_stack := stack_update (st_stack st) d imm |}
      | IAssumeValue _ _ => st
      | IExit => st
      end.
